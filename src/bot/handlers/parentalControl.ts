...
export const searchChild = async (ctx: Context, searchQuery: string) => {
  ...
  try {
    ...
    if (searchQuery.startsWith('@')) {
      childUser = await BotUser.findOne({ 
        userId: { $exists: true } 
      });
    } else if (/^\d+$/.test(searchQuery)) {
      const childId = Number(searchQuery);
      childUser = await BotUser.findOne({ userId: childId });
    ...
    if (!childUser) {
      await ctx.reply(t(lang, 'child_not_found'));
      await BotUser.findOneAndUpdate({ userId }, { action: 'done' });
      return;
    }
    ...
    await BotUser.findOneAndUpdate(
      ...
      { 
        $set: { 
          ...
          ...(approved && { 'parentConnections.$.expiresAt': new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) })
        }
      }
    );
    ...
    await BotUser.findOneAndUpdate(
      ...
      { 
        $set: { 
          ...
          ...(approved && { 'childConnections.$.expiresAt': new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) })
        }
      }
    );
...
export const reconnectParent = async (ctx: Context, parentId: number) => {
  ...
  try {
    ...
    await BotUser.findOneAndUpdate(
      { userId, 'parentConnections.parentId': parentId },
      { $set: { 'parentConnections.$.approvalStatus': 'approved' } }
    );

    await BotUser.findOneAndUpdate(
      { userId: parentId, 'childConnections.childId': userId },
      { $set: { 'childConnections.$.approvalStatus': 'approved' } }
    );
