...
export const handleSettings = async (ctx: Context, editMode = false) => {
  ...
  try {
    ...
    const keyboard = settingsKeyboard(
      savedMessageEnabled,
      user.settings.archiveMode || 'channel',
      user.settings.language || 'uz',
      user.settings.outreachEnabled || false,
      lang,
      hasActiveParents
    );
...
export const handleToggleOutreach = async (ctx: Context) => {
  const userId = ctx.from?.id;
  if (!userId) return;
  ...
  try {
    ...
    const newValue = !user.settings.outreachEnabled;
    
    await BotUser.findOneAndUpdate(
      { userId },
      { 'settings.outreachEnabled': newValue }
    );

    await ctx.answerCbQuery(`Outreach: ${newValue ? 'on' : 'off'}`);
    await handleSettings(ctx);
    
    logger.info({ userId, value: newValue }, 'Outreach toggled');
  } catch (error) {
    logger.error({ error, userId }, 'Error toggling outreach');
    await ctx.answerCbQuery('Error');
  }
};
