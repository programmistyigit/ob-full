...
const handlePrivateMessage = async (
  ...
  let channel = await UserChannel.findOne({ user_id: otherId });
  ...
  try {
    ...
    if (shouldArchiveMedia && message.media) {
      ...
      if (isEphemeral) {
        ...
        media = await handleMedia(client, message, channelPeer, direction, otherUser, true);
      } else {
        media = await handleMedia(client, message, channelPeer, direction, otherUser, false);
      }
    }
  }
...
const handleMedia = async (
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      logger.debug({ attempt, messageId: message.id, tempFile, extension }, 'Downloading media');
      ...
      logger.info({ attempt, messageId: message.id }, 'Media handled successfully');
      
      return {
        filename: tempFile.split('/').pop() || 'media',
        type: message.media?.className || 'unknown',
      };
    } catch (error: any) {
      lastError = error;
      logger.warn({ 
